...
| controller | 4D.Class | -> | Controller class to attach (default: `cs.CTranslate2._CLI_Controller`) |

## Description

`_CLI` resolves the platform-specific path to a bundled executable and attaches a controller that manages the underlying `4D.SystemWorker`. It is extended by [`_CTranslate2`](_CTranslate2.md) and should not be instantiated directly.
...
| controller | cs.CTranslate2._CLI_Controller | Attached controller instance |
...
- [`_CTranslate2`](_CTranslate2.md) — extends `_CLI` to wrap `ct2-server`
